...
def main():
    ...
    run_tmux_command(
        ["send-keys", "-t", f"{session_name}:1", "pact; nvim; clear", "C-m"]
    )

    # Window 2: servers (split first, then send commands)
    run_tmux_command(
        ["new-window", "-t", f"{session_name}:2", "-n", "servers", "-c", str(work_dir)]
    )
    run_tmux_command(
        ["split-window", "-v", "-t", f"{session_name}:2", "-c", str(work_dir)]
    )
    # Target panes 1 and 2 of window 2
    run_tmux_command(
        ["send-keys", "-t", f"{session_name}:2.1", "pact; clear; clear", "C-m"]
    )
    run_tmux_command(
        ["send-keys", "-t", f"{session_name}:2.2", "pact; clear; clear", "C-m"]
    )

    # Window 3: agent
    run_tmux_command(
        ["new-window", "-t", f"{session_name}:3", "-n", "agent", "-c", str(work_dir)]
    )
    run_tmux_command(["send-keys", "-t", f"{session_name}:3", "pact; clear", "C-m"])

    # Window 4: working
    run_tmux_command(
        ["new-window", "-t", f"{session_name}:4", "-n", "working", "-c", str(work_dir)]
    )
    run_tmux_command(["send-keys", "-t", f"{session_name}:4", "pact; clear", "C-m"])

    # Window 5: ssh
    run_tmux_command(
        ["new-window", "-t", f"{session_name}:4", "-n", "ssh", "-c", str(work_dir)]
    )
    run_tmux_command(["send-keys", "-t", f"{session_name}:4", "pact; clear", "C-m"])
